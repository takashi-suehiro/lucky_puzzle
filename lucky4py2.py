...
#       self.button3.pack(side=BOTTOM, fill=X)
    ...
    def init(self) :
        ...
        # define parts
        self.unit = 30
        self.v_tri1 = map(self.mul_unit,[0,0,2,0,0,2])
        self.v_tri2 = map(self.mul_unit,[2,0,4,0,4,2])
        self.v_nife1 = map(self.mul_unit,[0,2,2,0,3,1,0,4])
        self.v_nife2 = map(self.mul_unit,[2,2,3,1,4,2,4,4])
        self.v_nife3 = map(self.mul_unit,[0,4,1,3,1,5,0,5])
        self.v_nife4 = map(self.mul_unit,[3,3,4,4,4,5,3,5])
        self.v_house = map(self.mul_unit,[1,3,2,2,3,3,3,5,1,5])
        self.parts = {}
